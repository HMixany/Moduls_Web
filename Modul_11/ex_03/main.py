...
import sqlalchemy
from fastapi import FastAPI, Path, Query, Depends, HTTPException, status, Path, Request, File, UploadFile
from sqlalchemy.orm import Session

from db import get_db
from middlewares import AddProcessTimeHeader
from models import Owner, Cats
...
async def get_cat_by_id(cat_id: int = Path(ge=1), db: Session = Depends(get_db)):
    cat = db.query(Cats).filter_by(id=cat_id).first()
    if cat is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="NOT FOUND")
    return cat
